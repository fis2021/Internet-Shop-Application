...
test('Logout customer', async () => {
    ...
    const resLogout = await request(app).put('/api/auth/logout').send(
    {
        "access_token" : token
    }
    )

    expect(resLogout.statusCode).toEqual(201)
    expect(resLogout.body).toStrictEqual({})
})


test('Logout seller', async () => {
    await request(app).put('/api/auth/register').send({
        "user-type" : "seller",
        "user" : {
...
test('Logout undefined session', async () => {
     const res = await request(app).put('/api/auth/register').send({
        "access_token" : "7af587db-8aec-4f77-8d3a-8363fc827556"
    })
